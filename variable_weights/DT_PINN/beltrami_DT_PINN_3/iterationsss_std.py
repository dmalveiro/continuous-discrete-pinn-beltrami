...
def run_iterations():
    ...
    for prm.it in range(niter):
        ...
        prm.time_step = 0
        print("\nTIME = ", prm.time, "\n")
#        plots1.printall(data_a, prm.time_step, prm.it)

        # 1st stage training
        k = 0.1**(1/10000)
        model_a.compile(opt1, lr=lrate1_a, loss_weights=lossw1_a, decay=("exponential", k))
        start = time.perf_counter()
        losshistory, train_state = model_a.train(iterations=nepochs1_a, display_every=stepoch)
        elapsed = time.perf_counter() - start

        # 2nd stage training (L-BFGS) - optional
#        dde.optimizers.config.LBFGS_options["iter_per_step"] = stepoch
#        dde.optimizers.config.set_LBFGS_options(maxcor=100, ftol=0, gtol=1e-08, maxiter=nepochs2_a, maxfun=None, maxls=50)
#        model_a.compile(opt2, loss_weights=lossw2_a)
#        losshistory, train_state = model_a.train()

        # Save loss history
        dde.saveplot(losshistory, train_state, issave=True, isplot=True)
        os.rename("loss.dat", f"loss_it{prm.it}_ts{prm.time_step:04d}.dat")
        ...
        # Store training time
        store_data.save_training_times(prm.ib, prm.it, elapsed, niter, prm.time_step)

        # Store the data produced at 'results.pred_exact' in csv files
        store_data.in_variables(prm.ib, prm.it, xyz,
                                l2_difference_u, l2_difference_v, l2_difference_w, l2_difference_p, residual,
...
#            plots1.printall(data_b, prm.time_step, prm.it)

            # 1st stage training
            k_b = 0.1 ** (1/300)
            model_b.compile(opt1, lr=lrate1_b, loss_weights=lossw1_b, decay=("exponential", k_b))
            start = time.perf_counter()
            losshistory, train_state = model_b.train(iterations=nepochs1_b, display_every=stepoch)
            elapsed = time.perf_counter() - start

            # 2nd stage training (L-BFGS) - optional
#            dde.optimizers.config.LBFGS_options["iter_per_step"] = stepoch
#            dde.optimizers.config.set_LBFGS_options(maxcor=100, ftol=0, gtol=1e-08, maxiter=nepochs2_b, maxfun=None, maxls=50)
