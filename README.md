...
In V-PINN, the training set `training_set_run{r}_ORI.csv` present in the respective `RUN_1000_*` folder is extracted for the current run `r`. Then, the problem definition is set: 3D transient Navier-Stokes equations subject to constraints: a space-time domain, boundary and initial conditions (Beltrami flow analytical solutions in the domain boundaries and at t=0, respectively). The PINN architecture is then defined and the model (`model`) is composed by the problem (`data`) plus the network (`net`). The PINN is then trained for a user defined number of iterations (i.e., number of times the network weights are updated) and the loss history is saved. 

For each time step (or instant) `i`: the function `pred_exact()` is called, where the velocity and pressure fields are predicted in the test set from the trained model and these results are evaluated by computing the residuals and L2 relative errors between the predicted and exact results in the test set. The residuals calculation involves computing many derivatives through automatic differentiation (through the `pde()` function), which is a memory-consuming task, whose usage increases with the number of data points where these gradients are computed. As such, the three-dimensional test set (with `nx` * `ny` * `nz` points) is divided into `ny` 2D sub-domains (or slices) of `nx` * `nz` points. The residuals are then calculated in each sub-domain, are stored in memory, and the cached data regarding hessians and jacobians is clean (`dde.grad.clear()`), in a sequential process, avoiding data accumulation across the `ny` loops, saving memory resources compared to standard residuals calculation without domain slicing.

When all time steps are addressed (until `n_dt` time steps), average values of the evaluated metrics are stored in `final_data` file, and the execution ends. Optionally, the predicted, exact, relative errors and residuals at the test points can also be saved in csv files, which can be useful for visualizing results in software such as [Paraview](https://www.paraview.org/).

### DT-PINN execution

Regarding the DT-PINN version, the code execution is divided into two stages: for initial time (t=0) and remaining time steps (t>0). Similarlly to V-PINN, the whole code is executed `nrun` times. The training set `training_set_run{r}_ORI.csv` present in the respective `RUN_1000_*` folder is extracted for the current run `r`, for `model_a`, corresponding to t=0. The initial-time problem is set: fitting the analytical solution at t = 0. The full model (`model_a`) is, the neural network (standard MLP, `net_a`) plus the problem to optimize (`data_a`), which is then trained. 
