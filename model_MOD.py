...
# Most recent date of modification: 2026-08-23.
#
...
try:
    ...
    prm = None # Fallback if running a script that does not use prm.py
try:
    import param
except ImportError:
    param = None # Fallback if running a script that does not use param.py
...
class Model:
    ...
    def _compile_pytorch(self, lr, loss_fn, decay):
        ...
        def early_stopping1(losses):
            if prm is not None: source = prm
            else: source = param
            total_test_loss = torch.sum(losses)
            current_iteration = self.train_state.epoch
            if current_iteration > 0 and source.time_step > 0:
                target_loss = 1e-5
                if total_test_loss < target_loss:
                    self.stop_training = True
                    print(f"\nEarly Stopping because the test loss is below the target loss.")
                    print(f"Step: {current_iteration} | Total test loss = {total_test_loss:.6e} < target ({target_loss:.6e})")

        def early_stopping2(losses):
            if prm is not None: source = prm
            else: source = param
            total_test_loss = torch.sum(losses)
            current_iteration = self.train_state.epoch
            ...
            if current_iteration > 0:
                ...
                relative_delta_loss = (current_loss - source.previous_loss) / source.previous_loss
                if relative_delta_loss > epsilon:
                    self.stop_training = True
                    print(f"\nEarly Stopping because the test loss plateaued.")
                    print(f"Step: {current_iteration} | Relative loss variation = {relative_delta_loss:.6e} > epsilon ({epsilon:.6e})")
            source.previous_loss = total_test_loss

        def early_stopping3(total_loss):
            if prm is not None: source = prm
            else: source = param
            current_iteration = self.train_state.epoch
            if current_iteration > 0 and source.time_step > 0:
                ...
                if total_loss < target_loss:
                    self.stop_training = True
                    print(f"\nEarly Stopping because the training loss is below the target loss.")
                    print(f"Step: {current_iteration} | Total training loss = {total_loss:.6e} < target ({target_loss:.6e})")

        #####################################

        #### MOD #########################
        def compute_weights(losses):
            option = 0	# 1 for maximum gradient method
            num_bcs = 4	# number of PDE residual loss terms
            params = [p for p in self.net.parameters() if p.requires_grad]
            current_weight_list = []
            if option == 1: maxxx_abs_grads = torch.tensor(0)
            for i in range(len(losses)):
                grad_loss = torch.autograd.grad(losses[i], params, retain_graph=True)
                flattened_grads = [g.detach().flatten() for g in grad_loss if g is not None]
                avg_abs_grads = torch.mean(torch.abs(torch.cat(flattened_grads)))
                if option != 1:		# average gradient method
                    if i == 0: avg_abs_grads0 = avg_abs_grads
                    current_weight = avg_abs_grads0 / avg_abs_grads
                else:			# maximum gradient method
                    if i < num_bcs:	# 'num_bcs' PDE residuals
                        max_abs_grads = torch.max(torch.abs(torch.cat(flattened_grads)))
                        if max_abs_grads > maxxx_abs_grads: maxxx_abs_grads = max_abs_grads
                        current_weight = torch.tensor(1)
                    else: current_weight = avg_abs_grads / maxxx_abs_grads
                current_weight_list.append(current_weight)
            current_weight_list = torch.stack(current_weight_list)
            return current_weight_list
        ##################################
        ...
        def outputs_losses(training, inputs, targets, auxiliary_vars, losses_fn):
            ...
            if prm is not None:		# DT-PINNs
                source = prm
                current_step_dtpinn = prm.time_step
                niters1_a = prm.niters1_a
                niters1_b = prm.niters1_b
                niters2_a = prm.niters2_a
                niters2_b = prm.niters2_b
            else:	# V-PINNs
                source = param
                current_step_dtpinn = 1
            current_iteration = self.train_state.epoch
            grad_loss = None
            alpha = 0.1
            checkpoint = 100
            # Weighted losses
            if current_step_dtpinn == 0 and self.loss_weights is not None:
                losses *= torch.as_tensor(self.loss_weights)
            elif sig == 1:	# compute gradients when training ...
                if current_iteration % checkpoint == 0 and self.opt_name != "L-BFGS":	# ... and in intervals of 'checkpoint' iterations and if not in L-BFGS
                    source.current_weight_list = compute_weights(losses)
                    if current_iteration == 0: source.balanced_weight_list = source.current_weight_list
                    else: source.balanced_weight_list = (1 - alpha) * source.past_weight_list + alpha * source.current_weight_list
                    losses *= source.balanced_weight_list
                    source.past_weight_list = source.balanced_weight_list		# past weight list for the next iteration
                else:
                    losses *= source.balanced_weight_list
            if l1_factor > 0:
            ...
            ok1 = 0
            ok2 = 0
            if training == False and ok1 == 1: early_stopping1(losses)
            if training == False and ok2 == 1 and current_step_dtpinn > 0: early_stopping2(losses)
            ########
            return outputs_, losses
        ...
        def train_step(inputs, targets, auxiliary_vars):
            def closure():
                ...
                #####
                ok3 = 0
                if ok3 == 1: early_stopping3(total_loss)
                #####
                self.opt.zero_grad()
                total_loss.backward()
                return total_loss
