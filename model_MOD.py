...
class Model:
    ...
    def _compile_pytorch(self, lr, loss_fn, decay):
        ...
        def early_stopping1(losses):
            ...
            current_epoch = self.train_state.epoch
            if current_epoch > 0 and source.time_step > 0:
                target_loss = 1e-5
                if total_test_loss < target_loss:
                    self.stop_training = True
                    print(f"\nEarly Stopping because the test loss is below the target loss.")
                    print(f"Step: {current_epoch} | Total test loss = {total_test_loss:.6e} < target ({target_loss:.6e})")
        ...
        def early_stopping2(losses):
            ...
            if current_epoch > 0:
                ...
                if relative_delta_loss > epsilon:
                    ...
                    print(f"Step: {current_epoch} | Relative loss variation = {relative_delta_loss:.6e} > epsilon ({epsilon:.6e})")
        ...
        def early_stopping3(total_loss):
            ...
            if current_epoch > 0 and source.time_step > 0:
                ...
                if total_loss < target_loss:
                    ...
                    print(f"Step: {current_epoch} | Total training loss = {total_loss:.6e} < target ({target_loss:.6e})")
        ...
        def outputs_losses(training, inputs, targets, auxiliary_vars, losses_fn):
            ...
            if prm is not None:		# DT-PINNs
                ...
                nepochs1_a = prm.nepochs1_a
                nepochs1_b = prm.nepochs1_b
                nepochs2_a = prm.nepochs2_a
                nepochs2_b = prm.nepochs2_b
            else:	# V-PINNs
                source = param
            ...
            if current_step_dtpinn == 0 and self.loss_weights is not None:
                ...
                if current_epoch % checkpoint == 0 and self.opt_name != "L-BFGS":	# ... and in epoch intervals of checkpoint and if not in L-BFGS
                    source.current_weight_list = compute_weights(losses)
                    if current_epoch == 0: source.balanced_weight_list = source.current_weight_list
                    ...
                    source.past_weight_list = source.balanced_weight_list		# past weight list for the next epoch
                else:
                    losses *= source.balanced_weight_list
            if l1_factor > 0:
